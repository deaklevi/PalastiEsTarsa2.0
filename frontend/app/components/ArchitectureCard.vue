...
// Nuxt beépített useFetch használata Store/Axios helyett
const { data: response, pending: loading, error } = await useFetch(`${baseUrl}/api/architectures`, {
  key: `architectures-${props.group}`,
  watch: [() => props.group]
})
...
// Billentyűzet események (Jobb, Bal, Esc)
...
<template>
  <div class="flex flex-col items-center mb-24 md:mb-52">
    ...
    <div v-else-if="filteredArchitectures.length === 0" class="mt-10 text-gray-400 italic">Ebben a kategóriában jelenleg nincsenek feltöltött képek.</div>

    <div v-else class="flex flex-wrap justify-center gap-6 max-w-[1500px]">
      <div 
        v-for="(item, index) in filteredArchitectures" :key="item.id" 
      ...
      >
        ...
        <h5 class="text-[11px] md:text-sm text-center break-words mt-3 text-black font-medium leading-tight px-1">
          <span class="text-orange-600 font-bold uppercase">{{ item.order }}</span>
            ...
            <div class="w-full lg:w-2/5 p-5 md:p-8 flex flex-col justify-between bg-neutral-900 overflow-y-auto">
              <div>
                ...
                <div class="text-gray-300 text-xs md:text-sm leading-relaxed overflow-y-auto max-h-[15vh] lg:max-h-[40vh] mb-6 pr-2 custom-scrollbar">
                  Kategória: <span class="text-orange-500 font-bold uppercase">{{ selectedItem.group }}</span>
                  <br><br>
                  Ez az építészeti elem kiváló minőségű megmunkálással készült, egyedi méretben is rendelhető. További részletekért kérjük, vegye fel velünk a kapcsolatot!
                </div>
              </div>
